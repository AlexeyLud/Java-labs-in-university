...
public class NewClass {
    ...
    static void insert(Connection conn, PreparedStatement ps){
        try {
            // INSERT
            System.out.println("Запрос INSERT... tranname");
            String sql_ins2 = "INSERT INTO tranname (name, number) VALUES (?,?)";
            ps = conn.prepareStatement(sql_ins2);
            ps.setString(1, "Трамвай");
            ps.setInt(2, 12);
            int rowsInserted2 = ps.executeUpdate();
            if (rowsInserted2 > 0) {
                System.out.println("Новый транспорт упешно добавлен!");
            }
            System.out.println("-----------------------------------");
            
            System.out.println("Запрос INSERT... route");
            String sql_ins = "INSERT INTO route (startm, endm) VALUES (?, ?)";
            ps = conn.prepareStatement(sql_ins);
            ps.setString(1, "Технич. Универ");
            ps.setString(2, "Завод");
            int rowsInserted = ps.executeUpdate();
            if (rowsInserted > 0) {
                System.out.println("Новый маршрут упешно добавлен!");
            }
            System.out.println("-----------------------------------");
    ...
    static void select(Connection conn, ResultSet res, Statement s){
        try {
            // SELECT
            System.out.println("Запрос SELECT... tranname");
            ...
            while (res.next()){
                ...
                System.out.println("Номер транспорта: " + number);
                count2++;
            }
            System.out.println("Количество транспортов = " + count2);
            System.out.println("-----------------------------------");
            
            System.out.println("Запрос SELECT... route");
            ...
            while (res.next()){
                ...
                System.out.println("Начало маршрута: " + startm);
                System.out.println("Конец маршрута: " + endm);
                count++;
            }
            ...
            while (res.next()){
                ...
                System.out.println("Стоимость билета: " + coun);
                count3++;
            }
            ...
            while (res.next()){
                ...
                System.out.println("Начало времени работы: " + startw);
                System.out.println("Конец времени работы: " + endw);
                count4++;
            }
    ...
    static void update(Connection conn, PreparedStatement ps){
        try {
            // UPDATE
            System.out.println("Запрос UPDATE... tranname");
            String sql_up2 = "UPDATE tranname SET name=?, number=? WHERE name=?";
            ps = conn.prepareStatement(sql_up2);
            ps.setString(1, "Тролейбус");
            ps.setInt(2, 3);
            ps.setString(3, "Трамвай");
            int rowsUpdated2 = ps.executeUpdate();
            if (rowsUpdated2 > 0) {
                System.out.println("Данные транспорта успешно изменены!");
            }
            System.out.println("-----------------------------------");
            
            System.out.println("Запрос UPDATE... route");
            String sql_up = "UPDATE route SET startm=?, endm=? WHERE id_r=?";
            ps = conn.prepareStatement(sql_up);
            ps.setString(1, "Обл. поликлиника");
            ps.setString(2, "пр. Машерова");
            ps.setInt(3, 1);
            int rowsUpdated = ps.executeUpdate();
            if (rowsUpdated > 0) {
                System.out.println("Данные маршура успешно изменены!");
            }
            System.out.println("--------------------------------------------------------");
            System.out.println();
