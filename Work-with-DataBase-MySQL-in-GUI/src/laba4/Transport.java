...
public class Transport extends javax.swing.JFrame {
    ...
    public void Load()
    {
        try 
        {
            pst = conn.prepareStatement("select * from transport");
            rs = pst.executeQuery();
            ...
            while(rs.next())
            {
                ...
                for(int i = 0; i <= c; i++)
                {
                    ...
                    v.add(rs.getString("startmar"));
                    v.add(rs.getString("endmar"));
                }
                d.addRow(v);
            }
        } catch (SQLException ex) {
    ...
    private void initComponents() {
        ...
        jLabel1.setText("Регистрация транспорта");
        ...
        jTable1.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "ID", "Название", "Номер", "Стоимость", "НачалоРаб", "КонецРаб", "ДатаРег", "НачалоМар", "КонецМар"
            }
        ));
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        try 
        {
            ...
            String sql_sel2 = "select countproezd, datareg from dataintro";
            s = conn.createStatement();
            ...
            String sql_sel3 = "select startmar, endmar from router";
            s = conn.createStatement();
            ...
            int l = 0;
            while (res3.next()){
                startmar = res3.getString("startmar");
                sm[l] = startmar;
                endmar = res3.getString("endmar");
                em[l] = endmar;
                l++;
            }
            ...
            for(int p = 0; p < count; p++){
                pst = conn.prepareStatement("insert into transport(name, number, count, starttime, endtime, datareg, startmar, endmar) values(?,?,?,?,?,?,?,?)");
                pst.setString(1, n[p]);
                pst.setInt(2, num[p]);
                pst.setDouble(3, c[p]);
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
        ...
        try 
        {
            ...
            pst = conn.prepareStatement("delete from transport where id_t=?");
            pst.setString(1, id);
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        {
            d.setRowCount(0);
            pst = conn.prepareStatement("delete from transport");
            int k = pst.executeUpdate();
            JOptionPane.showMessageDialog(this, "Данные успешно удалены");
        }catch (SQLException ex) {
            Logger.getLogger(Transport.class.getName()).log(Level.SEVERE, null, ex);
